    ...
    for meeting in scraped_data:
        ...
        if committee_name_filter != "All Meetings":
            if committee_name_filter:
                normalized_meeting_name = meeting["EventBodyName"].replace(
                    " & ", " and "
                )
        ...
        if not upcoming_only:
            ...
            if (
                skip_cancellations and "cancel" in meeting["EventBodyName"].lower()
            ):  # different spelling of Cancel...
                continue
...
def render_committee_page(committee_name, year, meetings=[], sidebar_items=[]):
    ...
    outfile = os.path.join(
        CURRENT_DIRECTORY, "../WebPage/website/{}/{}.html".format(year, slug)
    )
...
for year in YEARS:
    try:
        ...
        scraped_file = os.path.abspath(
            os.path.join(
                CURRENT_DIRECTORY,
                "../WebPage/website/scraped/Scraper{}.json".format(year),
            )
        )
...
for committee_name in COMMITTEES:
    ...
    for (year, scraped_data) in DATA_BY_YEAR.items():
        past_meetings = load_meetings(
            scraped_data, committee_name_filter=committee_name, skip_cancellations=True
        )
        render_committee_page(committee_name, year, meetings=past_meetings)

# generate symlinks for index.html files
index_path = os.path.abspath(
    os.path.join(CURRENT_DIRECTORY, "../WebPage/website/index.html")
)
