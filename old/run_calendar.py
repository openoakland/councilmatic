...
def get_args():
    ...
    parser.add_argument(
        "-sdt",
        "--start_dt",
        help="start_dt - fine grain date filter for start date (mm/dd/yyyy)",
        type=str,
        default=None,
    )
    parser.add_argument(
        "-edt",
        "--end_dt",
        help="end_dt - fine grain date filter for end date (mm/dd/yyyy)",
        type=str,
        default=None,
    )
    parser.add_argument(
...
def scrape(args):
    start_dt = (
        None if args.start_dt is None else datetime.strptime(args.start_dt, "%m/%d/%Y")
    )
