...
def scrape_api(days, year, meeting_file):
    ...
    for meeting in tqdm.tqdm(meetings):
        try:
            agenda = requests.get(
                API_URL
                + "Events/{}/EventItems?AgendaNote=1&MinutesNote=1&Attachments=1".format(
                    meeting["EventId"]
                )
            ).json()
            ...
            for item in agenda:
                if item["EventItemVideo"] is not None:
                    meeting["EventVideoPath"] = (
                        "http://oakland.granicus.com/MediaPlayer.php?view_id=2&meta_id="
                        + str(item["EventItemVideo"])
                    )
                    break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--days",
        help="Number of previous days to retrieve meeting details for",
        type=int,
    )
    parser.add_argument(
        "--year", help="Year to retrieve events for (overrides days)", type=int
    )
